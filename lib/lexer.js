...
class Lexer {
  constructor(source, filename) {
    ...
    this.reserve(new Word('import', Tag.IMPORT));
    this.reserve(new Word('public', Tag.PUBLIC));
    this.reserve(new Word('weak', Tag.WEAK));
  ...
  scan() {
    ...
    if (isDecimalDigit(this.peek)) {
      if (this.peek !== '0') { // decimalLit
        ...
        return new PBNumber(parseInt(v, 8));
      }
    }
