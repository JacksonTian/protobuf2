...
class Parser {
  ...
  enum() {
    this.matchWord(Tag.DEFINITION, 'enum');
  ...
  messageBody() {
    ...
    var repeated = false;
    if (this.look.tag === Tag.ID && this.look.lexeme === 'repeated') {
      repeated = true;
      this.move();
    }
    ...
    if (this.look.tag === Tag.OPTION) {
      ...
      t = new Field(repeated, type, fieldName, fieldNumber, fieldOptions);
    } else if (this.look.tag === Tag.DEFINITION && this.look.lexeme === 'message') {
      t = this.message();
    } else {
      this.error();
    }
